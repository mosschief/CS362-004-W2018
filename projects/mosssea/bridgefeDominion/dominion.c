...
//refactored Adventurer,now its own function.
void playAdventurer(int currentPlayer, int temphand[], int z, struct gameState *state){
    int drawntreasure = 0;
    int cardDrawn;
    ...
    while(drawntreasure<2){
        ...
        drawCard(currentPlayer, state);//---BUG--- now draws an extra card by calling drawCard twice.
...
//refactored council room, now its own function.
void playCouncilRoom(int currentPlayer, int handPos, struct gameState *state){
    int i;
    //+2 Cards
    for (i = 0; i < 3; i++)//--BUG-- Now only allows player to draw 2 cards instead of 3
    {
        drawCard(currentPlayer, state);
    }
...
//refactored smith card into its own function
...
//refactored great hall card into its own function
void playGreatHall(int currentPlayer, int handPos, struct gameState *state){
    //+1 Card
    drawCard(currentPlayer, state);
    drawCard(currentPlayer, state);//_--BUG-- This Draws two cards rather than 1 by calling drawCard an extra time
...
void playSteward(int currentPlayer, int handPos, int choice1, int choice2,
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    //refactored to call playAdventurer card function
    case adventurer:
            playAdventurer(currentPlayer, temphand, z, state);
            return 0;
	//refactored to call playcouncilroom function
    case council_room:
